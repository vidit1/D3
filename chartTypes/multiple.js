...
function D3Multiple(obj) {
  ...
  var margin = {top: 20, right: 50, bottom: 60, left: 50},
    ...
    height = obj.height ? obj.height : 550 - margin.top - margin.bottom,tickNumber=5
  ...
  //Defining yAxis location at left
  ...
  //Defining Secondary yAxis location right
  ...
  var line = d3.svg.line()
    .x(function (d) {
      return x(d.date)+ (x.rangeBand()-1)/2;
    })
    .y(function (d) {
      return y1(d.y);
    });
  ...
  var cities1 = d3.layout.stack()(data.filter(function (d,i) {
      return i <=location
    }).map(function (d) {
  ...
  var cities2 = data.filter(function (d,i) {
    return i >location
  })
  ...
  for (var i = 0; i < cities.length; i++) {
    ...
    cityCopyObj.location = i<=location;
  ...
  chart.plot = function () {
    ...
    if(obj.exporting&&obj.hasOwnProperty('exporting')&&obj.exporting.buttons.hasOwnProperty('qkeyButton')){
    ...
    y.domain([
      0,
      d3.max(series.filter(function (d) {
        return d.location;
      }), function (c) {
    ...
    svg.append("g")
      ...
      .text(obj.yAxis[0] && obj.yAxis[0].title.text ? obj.yAxis[0].title.text : "");

    //Appending y - axis line
    svg.append("g")
      ...
      .style("text-anchor", "middle")
      .attr("class","axis-label")
      .text(obj.yAxis[1]&&obj.yAxis[1].title.text?obj.yAxis[1].title.text:"");
